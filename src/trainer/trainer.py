...
class Trainer_GAN_E2E(BasicTrainer):
    ...
    def _step(self, data):
        ...
        '''
        STEP 3 (training denoiser)
        '''
        # forward
        generated_noise_map = self.model['model_G'](data_CL['clean'])
        generated_noisy_img = data_CL['clean'] + generated_noise_map.detach()
        denoised_image = self.model['denoiser'](generated_noisy_img)

        # get losses for denoiser 
        ## here when only name of loss is in configuration, loss will be calculated.
        losses = {}
        losses.update(self.loss(denoised_image, data_CL, 'L1'))
        losses.update(self.loss(denoised_image, data_CL, 'L2'))
