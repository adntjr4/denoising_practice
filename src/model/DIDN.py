...
class DIDN(nn.Module):
    def __init__(self, in_ch, out_ch, n_DUB):
        self.n_DUB = n_DUB

        super().__init__()

        self.conv_input = nn.Conv2d(in_channels=in_ch, out_channels=256, kernel_size=3, stride=1, padding=1, bias=False)
        self.relu1 = nn.PReLU()
        ...
        self.conv_output = nn.Conv2d(in_channels=64, out_channels=out_ch, kernel_size=3, stride=1, padding=1, bias=False)
...
class DIDN_6(DIDN):
    def __init__(self, in_ch, out_ch):
        super().__init__(in_ch, out_ch, n_DUB=6)

@regist_model
class DIDN_8(DIDN):
    def __init__(self, in_ch, out_ch):
        super().__init__(in_ch, out_ch, n_DUB=8)
